...
Most websites use forms. Most websites need forms. In this article I will show how we have been using forms in decoupled using the [drupal graphql](https://www.drupal.org/project/graphql) module and [Next.Js](https://nextjs.org/). The concept would apply to any other framework as well.

## The idea / motivation

We have a lot of repositories and most of them share a lot of the code. This means that we re-use a lot of code for a lot of different purposes (for good and / or bad). As we started decoupling (way back in 2018) we knew forms was going to be a challenge and something we needed to solve.
